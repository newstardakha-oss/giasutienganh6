...
const unit2VocabList = JSON.parse(
  fs.readFileSync('d:\\đảng viên\\2025\\Viet apps\\Englishtutor 6\\src\\data\\unit2_vocab.json', 'utf8')
);

// Tạo câu hỏi Vocabulary cho 61 từ vựng Unit 2
const vocabQuestions = unit2VocabList.map((item) => {
  const options = [item.word];
  const others = unit2VocabList.filter((v) => v.word !== item.word).map((v) => v.word);
  ...
  return {
    ...
    skill: 'Vocabulary',
    type: 'fill-in-blank',
    content: `📌 [UNIT 2: MY HOME - ${item.part}]\n${blankContent}`,
    options: options,
...
if (targetRegex.test(content)) {
  ...
  console.log(`Successfully added ${vocabQuestions.length} Unit 2 Vocabulary questions to sgkData.ts!`);
} else {
