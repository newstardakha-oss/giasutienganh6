...
const unit5VocabList = JSON.parse(
  fs.readFileSync('d:\\đảng viên\\2025\\Viet apps\\Englishtutor 6\\src\\data\\unit5_vocab.json', 'utf8')
);

// Tạo câu hỏi Vocabulary cho 80 từ vựng Unit 5
const vocabQuestions = unit5VocabList.map((item) => {
  const options = [item.word];
  const others = unit5VocabList.filter((v) => v.word !== item.word).map((v) => v.word);
  ...
  return {
    ...
    skill: 'Vocabulary',
    type: 'fill-in-blank',
    content: `📌 [UNIT 5: NATURAL WONDERS OF VIET NAM - ${item.part}]\n${blankContent}`,
    options: options,
...
if (targetRegex.test(content)) {
  ...
  console.log(`Successfully added ${vocabQuestions.length} Unit 5 Vocabulary questions to sgkData.ts!`);
} else {
