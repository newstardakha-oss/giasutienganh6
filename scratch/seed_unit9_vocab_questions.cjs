...
const unit9VocabList = JSON.parse(
  fs.readFileSync('d:\\đảng viên\\2025\\Viet apps\\Englishtutor 6\\src\\data\\unit9_vocab.json', 'utf8')
);

// Tạo câu hỏi Vocabulary cho 72 từ vựng Unit 9
const vocabQuestions = unit9VocabList.map((item) => {
  const options = [item.word];
  const others = unit9VocabList.filter((v) => v.word !== item.word).map((v) => v.word);
  ...
  return {
    ...
    skill: 'Vocabulary',
    type: 'fill-in-blank',
    content: `📌 [UNIT 9: CITIES OF THE WORLD - ${item.part}]\n${blankContent}`,
    options: options,
...
if (targetRegex.test(content)) {
  ...
  console.log(`Successfully added ${vocabQuestions.length} Unit 9 Vocabulary questions to sgkData.ts!`);
} else {
