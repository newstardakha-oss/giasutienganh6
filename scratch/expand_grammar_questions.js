...
const tsItems = questionsData.map((q) => {
...
const insertedTs = ',\n' + tsItems.join(',\n');
const target = 'export const INITIAL_PROGRESS = {';

if (content.includes(target)) {
  const newContent = content.replace(target, `${insertedTs}\n];\n\n${target}`);
  fs.writeFileSync(filePath, newContent, 'utf8');
  console.log(`Successfully added ${questionsData.length} new grammar questions to sgkData.ts!`);
} else {
