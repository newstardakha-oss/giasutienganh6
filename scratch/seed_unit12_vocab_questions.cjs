...
const unit12VocabList = JSON.parse(
  fs.readFileSync('d:\\đảng viên\\2025\\Viet apps\\Englishtutor 6\\src\\data\\unit12_vocab.json', 'utf8')
);

// Tạo câu hỏi Vocabulary cho 46 từ vựng Unit 12
const vocabQuestions = unit12VocabList.map((item) => {
  const options = [item.word];
  const others = unit12VocabList.filter((v) => v.word !== item.word).map((v) => v.word);
  ...
  return {
    ...
    skill: 'Vocabulary',
    type: 'fill-in-blank',
    content: `📌 [UNIT 12: ROBOTS - ${item.part}]\n${blankContent}`,
    options: options,
...
if (targetRegex.test(content)) {
  ...
  console.log(`Successfully added ${vocabQuestions.length} Unit 12 Vocabulary questions to sgkData.ts!`);
} else {
