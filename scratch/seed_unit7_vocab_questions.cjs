...
const unit7VocabList = JSON.parse(
  fs.readFileSync('d:\\đảng viên\\2025\\Viet apps\\Englishtutor 6\\src\\data\\unit7_vocab.json', 'utf8')
);

// Tạo câu hỏi Vocabulary cho 68 từ vựng Unit 7
const vocabQuestions = unit7VocabList.map((item) => {
  const options = [item.word];
  const others = unit7VocabList.filter((v) => v.word !== item.word).map((v) => v.word);
  ...
  return {
    ...
    skill: 'Vocabulary',
    type: 'fill-in-blank',
    content: `📌 [UNIT 7: TELEVISION - ${item.part}]\n${blankContent}`,
    options: options,
...
if (targetRegex.test(content)) {
  ...
  console.log(`Successfully added ${vocabQuestions.length} Unit 7 Vocabulary questions to sgkData.ts!`);
} else {
