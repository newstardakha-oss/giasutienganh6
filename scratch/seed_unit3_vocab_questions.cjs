...
const unit3VocabList = JSON.parse(
  fs.readFileSync('d:\\đảng viên\\2025\\Viet apps\\Englishtutor 6\\src\\data\\unit3_vocab.json', 'utf8')
);

// Tạo câu hỏi Vocabulary cho 65 từ vựng Unit 3
const vocabQuestions = unit3VocabList.map((item) => {
  const options = [item.word];
  const others = unit3VocabList.filter((v) => v.word !== item.word).map((v) => v.word);
  ...
  return {
    ...
    skill: 'Vocabulary',
    type: 'fill-in-blank',
    content: `📌 [UNIT 3: MY FRIENDS - ${item.part}]\n${blankContent}`,
    options: options,
...
if (targetRegex.test(content)) {
  ...
  console.log(`Successfully added ${vocabQuestions.length} Unit 3 Vocabulary questions to sgkData.ts!`);
} else {
