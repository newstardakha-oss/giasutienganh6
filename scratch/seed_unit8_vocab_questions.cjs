...
const unit8VocabList = JSON.parse(
  fs.readFileSync('d:\\đảng viên\\2025\\Viet apps\\Englishtutor 6\\src\\data\\unit8_vocab.json', 'utf8')
);

// Tạo câu hỏi Vocabulary cho 70 từ vựng Unit 8
const vocabQuestions = unit8VocabList.map((item) => {
  const options = [item.word];
  const others = unit8VocabList.filter((v) => v.word !== item.word).map((v) => v.word);
  ...
  return {
    ...
    skill: 'Vocabulary',
    type: 'fill-in-blank',
    content: `📌 [UNIT 8: SPORTS AND GAMES - ${item.part}]\n${blankContent}`,
    options: options,
...
if (targetRegex.test(content)) {
  ...
  console.log(`Successfully added ${vocabQuestions.length} Unit 8 Vocabulary questions to sgkData.ts!`);
} else {
