...
const unit11VocabList = JSON.parse(
  fs.readFileSync('d:\\đảng viên\\2025\\Viet apps\\Englishtutor 6\\src\\data\\unit11_vocab.json', 'utf8')
);

// Tạo câu hỏi Vocabulary cho 57 từ vựng Unit 11
const vocabQuestions = unit11VocabList.map((item) => {
  const options = [item.word];
  const others = unit11VocabList.filter((v) => v.word !== item.word).map((v) => v.word);
  ...
  return {
    ...
    skill: 'Vocabulary',
    type: 'fill-in-blank',
    content: `📌 [UNIT 11: OUR GREENER WORLD - ${item.part}]\n${blankContent}`,
    options: options,
...
if (targetRegex.test(content)) {
  ...
  console.log(`Successfully added ${vocabQuestions.length} Unit 11 Vocabulary questions to sgkData.ts!`);
} else {
