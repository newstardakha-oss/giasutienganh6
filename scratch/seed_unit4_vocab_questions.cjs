...
const unit4VocabList = JSON.parse(
  fs.readFileSync('d:\\đảng viên\\2025\\Viet apps\\Englishtutor 6\\src\\data\\unit4_vocab.json', 'utf8')
);

// Tạo câu hỏi Vocabulary cho 58 từ vựng Unit 4
const vocabQuestions = unit4VocabList.map((item) => {
  const options = [item.word];
  const others = unit4VocabList.filter((v) => v.word !== item.word).map((v) => v.word);
  ...
  return {
    ...
    skill: 'Vocabulary',
    type: 'fill-in-blank',
    content: `📌 [UNIT 4: MY NEIGHBOURHOOD - ${item.part}]\n${blankContent}`,
    options: options,
...
if (targetRegex.test(content)) {
  ...
  console.log(`Successfully added ${vocabQuestions.length} Unit 4 Vocabulary questions to sgkData.ts!`);
} else {
