 ...
 * @param unitId "unit-2", "unit-3", "unit-4", hoặc "unit-5" (mặc định "unit-5")
 * @param part Tên phần bài học (VD: "GETTING STARTED", "A CLOSER LOOK 1", "SKILLS 1"...)
 */
export function getVocabByPart(part: string, unitId: string = 'unit-5'): VocabItem[] {
  let sourceList = UNIT5_VOCAB_DATA;
 ...
 * @param unitId "unit-2", "unit-3", "unit-4", "unit-5"
 * @param part (Tùy chọn) Lọc theo phần bài học
 */
export function getRandomVocabForGames(count: number = 10, unitId: string = 'unit-5', part?: string): VocabItem[] {
 ...
 */
export function convertVocabToQuestion(vocab: VocabItem, unitId: string = 'unit-5'): Question {
  let sourceList = UNIT5_VOCAB_DATA;
  if (unitId === 'unit-2') sourceList = UNIT2_VOCAB_DATA;
  if (unitId === 'unit-3') sourceList = UNIT3_VOCAB_DATA;
  if (unitId === 'unit-4') sourceList = UNIT4_VOCAB_DATA;
