          ...
          {currentQ.options && currentQ.options.length > 0 ? (
            ...
            <div className="space-y-3 mb-4">
              <input
                ...
                disabled={showResult}
                className="w-full px-4 py-3 rounded-xl bg-slate-800 border border-slate-700 text-white font-semibold text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none"
              />
              <button
                type="button"
                disabled={showResult || !fillInput.trim()}
                onClick={() => handleAnswer(fillInput.trim())}
                className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-bold rounded-xl shadow-md transition-all flex items-center justify-center gap-2 text-sm"
              >
                <span>Chạy Tiến Bước (Xác Nhận Đáp Án)</span>
              </button>
            </div>
          )}
